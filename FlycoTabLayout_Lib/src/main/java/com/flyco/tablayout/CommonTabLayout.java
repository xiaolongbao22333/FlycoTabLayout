...
//        Log.d("AAA", "mCurrentP--->" + mCurrentP.left + "&" + mCurrentP.right);
    ...
    public List<TextView> getTitleViews() {
        List<TextView> listTab = new ArrayList<>();
        for (int i = 0; i < mTabsContainer.getChildCount(); i++) {
            View tabView = mTabsContainer.getChildAt(i);
            TextView tv_tab_title = (TextView) tabView.findViewById(R.id.tv_tab_title);
            listTab.add(tv_tab_title);
        }
        return listTab;
    }
     ...
     * @param size2 未选中自提大小
     */
    public void setSelectSize(int pos, float size1, float size2) {
        for (int i = 0; i < mTabsContainer.getChildCount(); i++) {
            View tabView = mTabsContainer.getChildAt(i);
            TextView tv_tab_title = (TextView) tabView.findViewById(R.id.tv_tab_title);
            if (pos == i) {
                tv_tab_title.setTextSize(size1);
            } else {
                tv_tab_title.setTextSize(size2);
            }
        }
